...
userRouter.post("/api/add-to-cart", auth, async (req, res) => {
    try {
        ...
        if (user.cart.length == 0) {
            ...
            if (isProductFound) {
                let producttt = await user.cart.find((productt) => productt.product._id.equals(product._id));
...
userRouter.get("/api/get-orders", auth, async (req, res) => {
    try {
        const orders = await Order.findById({ userId: req.user });
        res.json(orders);
    } catch (error) {
        res.status(500).json({
            error: e.message
